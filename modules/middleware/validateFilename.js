...
function filenameRedirect(req, res) {
  ...
  let resolvingTypes = req.query.types != null;
  if (resolvingTypes) {
    filename = req.packageConfig.types || req.packageConfig.typings;
    ...
    if (!filename) {
      if (req.packageName.startsWith('@types/')) {
        filename = 'index.d.ts';
      } else {
        return res
          .status(404)
          .type('text')
          .send(`Package ${req.packageSpec} does not contain types`);
      }
    }
  }
